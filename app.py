...
class InvoiceApp(tk.Tk):
    ...
    def _recognize_one(self, pdf_path: Path) -> dict[str, str]:
        try:
    ...
    def _run_recognition(self) -> None:
        ...
        completed = 0
        workers = min(8, total, (os.cpu_count() or 4) * 2)

        with concurrent.futures.ThreadPoolExecutor(max_workers=workers) as executor:
            futures = {
                executor.submit(self._recognize_one, pdf_path): pdf_path
            ...
            for future in concurrent.futures.as_completed(futures):
                ...
                self.after(
                    0,
                    lambda c=completed, t=total: self.status_var.set(
                        f"识别中 ({c}/{t})"
                    ),
                )
